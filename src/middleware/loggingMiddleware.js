...
const actionMappings = {
  ...
  "POST:/api/consultants": { actionType: "TÉLÉCHARGEMENT_CV", entityType: "Consultant" }, // Assuming this is the uploadCV route
...
const loggingMiddleware = async (req, res, next) => {
  // Skip logging for unauthenticated requests or if user ID is missing
  ...
  if (!actionInfo) {
    ...
    const actionType = `${defaultActionTypes[method] || "ACTION_INCONNUE"}_${entityType.toUpperCase()}`;
    ...
    try {
      ...
      switch (actionType) {
        ...
        case "SUPPRESSION_CRENEAU":
          description = `Créneau ${metadata.exchangeNumber || req.params.id} supprimé`;
          break;
